...
def drawer(object_list, place_to_draw_stuff=screen):
    for i in object_list:
        ...
        if r <= 1:
            r = 1
        p.draw.circle(place_to_draw_stuff, i.color, ball_xy(i), r)
...
# PUT SETUP CODE HERE ----------------------------------------------------------
system = System()
system.add(CelestialBody(Vec(-5, 0, 0), Vec(0,0,0), 1000))
system.add(CelestialBody(Vec(5, 0, 0), Vec(0,0,0), 1000))
system.update()
print("GRAVITY", force_due_to_gravity(system.directory[0], system.directory[0]))
